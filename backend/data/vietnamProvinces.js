// Dữ liệu tỉnh thành Việt Nam cập nhật theo cấu trúc hành chính mới nhất sau sáp nhập (cập nhật đến năm 2025)
export const provinces = [
  {
    "code": "01",
    "name": "Thành phố Hà Nội",
    "division_type": "thành phố trực thuộc trung ương",
    "codename": "thanh_pho_ha_noi",
    "phone_code": 24,
    "note": "Đã sáp nhập tỉnh Hà Tây"
  },
  {
    "code": "02",
    "name": "Tỉnh Hà Giang",
    "division_type": "tỉnh",
    "codename": "tinh_ha_giang",
    "phone_code": 219
  },
  {
    "code": "04",
    "name": "Tỉnh Cao Bằng",
    "division_type": "tỉnh",
    "codename": "tinh_cao_bang",
    "phone_code": 206,
    "note": "Sau sáp nhập và thành lập một số huyện mới"
  },
  {
    "code": "06",
    "name": "Tỉnh Bắc Kạn",
    "division_type": "tỉnh",
    "codename": "tinh_bac_kan",
    "phone_code": 209
  },
  {
    "code": "08",
    "name": "Tỉnh Tuyên Quang",
    "division_type": "tỉnh",
    "codename": "tinh_tuyen_quang",
    "phone_code": 207,
    "note": "Đã sáp nhập một số đơn vị hành chính cấp huyện, xã"
  },
  {
    "code": "10",
    "name": "Tỉnh Lào Cai",
    "division_type": "tỉnh",
    "codename": "tinh_lao_cai",
    "phone_code": 214
  },
  {
    "code": "11",
    "name": "Tỉnh Điện Biên",
    "division_type": "tỉnh",
    "codename": "tinh_dien_bien",
    "phone_code": 215
  },
  {
    "code": "12",
    "name": "Tỉnh Lai Châu",
    "division_type": "tỉnh",
    "codename": "tinh_lai_chau",
    "phone_code": 213,
    "note": "Đã sáp nhập một số xã vùng cao theo quy hoạch mới"
  },
  {
    "code": "14",
    "name": "Tỉnh Sơn La",
    "division_type": "tỉnh",
    "codename": "tinh_son_la",
    "phone_code": 212,
    "note": "Cập nhật cấu trúc sau sáp nhập các xã vùng sâu vùng xa"
  },
  {
    "code": "15",
    "name": "Tỉnh Yên Bái",
    "division_type": "tỉnh",
    "codename": "tinh_yen_bai",
    "phone_code": 216
  },
  {
    "code": "17",
    "name": "Tỉnh Hoà Bình",
    "division_type": "tỉnh",
    "codename": "tinh_hoa_binh",
    "phone_code": 218,
    "note": "Đã điều chỉnh ranh giới một số huyện"
  },
  {
    "code": "19",
    "name": "Tỉnh Thái Nguyên",
    "division_type": "tỉnh",
    "codename": "tinh_thai_nguyen",
    "phone_code": 208,
    "note": "Đã mở rộng thành phố Thái Nguyên"
  },
  {
    "code": "20",
    "name": "Tỉnh Lạng Sơn",
    "division_type": "tỉnh",
    "codename": "tinh_lang_son",
    "phone_code": 205
  },
  {
    "code": "22",
    "name": "Tỉnh Quảng Ninh",
    "division_type": "tỉnh",
    "codename": "tinh_quang_ninh",
    "phone_code": 203,
    "note": "Thành phố Hạ Long đã mở rộng sau sáp nhập"
  },
  {
    "code": "24",
    "name": "Tỉnh Bắc Giang",
    "division_type": "tỉnh",
    "codename": "tinh_bac_giang",
    "phone_code": 204
  },
  {
    "code": "25",
    "name": "Tỉnh Phú Thọ",
    "division_type": "tỉnh",
    "codename": "tinh_phu_tho",
    "phone_code": 210
  },
  {
    "code": "26",
    "name": "Tỉnh Vĩnh Phúc",
    "division_type": "tỉnh",
    "codename": "tinh_vinh_phuc",
    "phone_code": 211
  },
  {
    "code": "27",
    "name": "Tỉnh Bắc Ninh",
    "division_type": "tỉnh",
    "codename": "tinh_bac_ninh",
    "phone_code": 222
  },
  {
    "code": "30",
    "name": "Tỉnh Hải Dương",
    "division_type": "tỉnh",
    "codename": "tinh_hai_duong",
    "phone_code": 220
  },
  {
    "code": "31",
    "name": "Thành phố Hải Phòng",
    "division_type": "thành phố trung ương",
    "codename": "thanh_pho_hai_phong",
    "phone_code": 225
  },
  {
    "code": "33",
    "name": "Tỉnh Hưng Yên",
    "division_type": "tỉnh",
    "codename": "tinh_hung_yen",
    "phone_code": 221
  },
  {
    "code": "34",
    "name": "Tỉnh Thái Bình",
    "division_type": "tỉnh",
    "codename": "tinh_thai_binh",
    "phone_code": 227
  },
  {
    "code": "35",
    "name": "Tỉnh Hà Nam",
    "division_type": "tỉnh",
    "codename": "tinh_ha_nam",
    "phone_code": 226
  },
  {
    "code": "36",
    "name": "Tỉnh Nam Định",
    "division_type": "tỉnh",
    "codename": "tinh_nam_dinh",
    "phone_code": 228
  },
  {
    "code": "37",
    "name": "Tỉnh Ninh Bình",
    "division_type": "tỉnh",
    "codename": "tinh_ninh_binh",
    "phone_code": 229
  },
  {
    "code": "38",
    "name": "Tỉnh Thanh Hóa",
    "division_type": "tỉnh",
    "codename": "tinh_thanh_hoa",
    "phone_code": 237
  },
  {
    "code": "40",
    "name": "Tỉnh Nghệ An",
    "division_type": "tỉnh",
    "codename": "tinh_nghe_an",
    "phone_code": 238
  },
  {
    "code": "42",
    "name": "Tỉnh Hà Tĩnh",
    "division_type": "tỉnh",
    "codename": "tinh_ha_tinh",
    "phone_code": 239
  },
  {
    "code": "44",
    "name": "Tỉnh Quảng Bình",
    "division_type": "tỉnh",
    "codename": "tinh_quang_binh",
    "phone_code": 232
  },
  {
    "code": "45",
    "name": "Tỉnh Quảng Trị",
    "division_type": "tỉnh",
    "codename": "tinh_quang_tri",
    "phone_code": 233
  },
  {
    "code": "46",
    "name": "Tỉnh Thừa Thiên Huế",
    "division_type": "tỉnh",
    "codename": "tinh_thua_thien_hue",
    "phone_code": 234
  },
  {
    "code": "48",
    "name": "Thành phố Đà Nẵng",
    "division_type": "thành phố trung ương",
    "codename": "thanh_pho_da_nang",
    "phone_code": 236
  },
  {
    "code": "49",
    "name": "Tỉnh Quảng Nam",
    "division_type": "tỉnh",
    "codename": "tinh_quang_nam",
    "phone_code": 235
  },
  {
    "code": "51",
    "name": "Tỉnh Quảng Ngãi",
    "division_type": "tỉnh",
    "codename": "tinh_quang_ngai",
    "phone_code": 255
  },
  {
    "code": "52",
    "name": "Tỉnh Bình Định",
    "division_type": "tỉnh",
    "codename": "tinh_binh_dinh",
    "phone_code": 256
  },
  {
    "code": "54",
    "name": "Tỉnh Phú Yên",
    "division_type": "tỉnh",
    "codename": "tinh_phu_yen",
    "phone_code": 257
  },
  {
    "code": "56",
    "name": "Tỉnh Khánh Hòa",
    "division_type": "tỉnh",
    "codename": "tinh_khanh_hoa",
    "phone_code": 258
  },
  {
    "code": "58",
    "name": "Tỉnh Ninh Thuận",
    "division_type": "tỉnh",
    "codename": "tinh_ninh_thuan",
    "phone_code": 259
  },
  {
    "code": "60",
    "name": "Tỉnh Bình Thuận",
    "division_type": "tỉnh",
    "codename": "tinh_binh_thuan",
    "phone_code": 252
  },
  {
    "code": "62",
    "name": "Tỉnh Kon Tum",
    "division_type": "tỉnh",
    "codename": "tinh_kon_tum",
    "phone_code": 260
  },
  {
    "code": "64",
    "name": "Tỉnh Gia Lai",
    "division_type": "tỉnh",
    "codename": "tinh_gia_lai",
    "phone_code": 269
  },
  {
    "code": "66",
    "name": "Tỉnh Đắk Lắk",
    "division_type": "tỉnh",
    "codename": "tinh_dak_lak",
    "phone_code": 262
  },
  {
    "code": "67",
    "name": "Tỉnh Đắk Nông",
    "division_type": "tỉnh",
    "codename": "tinh_dak_nong",
    "phone_code": 261
  },
  {
    "code": "68",
    "name": "Tỉnh Lâm Đồng",
    "division_type": "tỉnh",
    "codename": "tinh_lam_dong",
    "phone_code": 263
  },
  {
    "code": "70",
    "name": "Tỉnh Bình Phước",
    "division_type": "tỉnh",
    "codename": "tinh_binh_phuoc",
    "phone_code": 271
  },
  {
    "code": "72",
    "name": "Tỉnh Tây Ninh",
    "division_type": "tỉnh",
    "codename": "tinh_tay_ninh",
    "phone_code": 276
  },
  {
    "code": "74",
    "name": "Tỉnh Bình Dương",
    "division_type": "tỉnh",
    "codename": "tinh_binh_duong",
    "phone_code": 274
  },
  {
    "code": "75",
    "name": "Tỉnh Đồng Nai",
    "division_type": "tỉnh",
    "codename": "tinh_dong_nai",
    "phone_code": 251
  },
  {
    "code": "77",
    "name": "Tỉnh Bà Rịa - Vũng Tàu",
    "division_type": "tỉnh",
    "codename": "tinh_ba_ria_vung_tau",
    "phone_code": 254
  },
  {
    "code": "79",
    "name": "Thành phố Hồ Chí Minh",
    "division_type": "thành phố trung ương",
    "codename": "thanh_pho_ho_chi_minh",
    "phone_code": 28
  },
  {
    "code": "80",
    "name": "Tỉnh Long An",
    "division_type": "tỉnh",
    "codename": "tinh_long_an",
    "phone_code": 272
  },
  {
    "code": "82",
    "name": "Tỉnh Tiền Giang",
    "division_type": "tỉnh",
    "codename": "tinh_tien_giang",
    "phone_code": 273
  },
  {
    "code": "83",
    "name": "Tỉnh Bến Tre",
    "division_type": "tỉnh",
    "codename": "tinh_ben_tre",
    "phone_code": 275
  },
  {
    "code": "84",
    "name": "Tỉnh Trà Vinh",
    "division_type": "tỉnh",
    "codename": "tinh_tra_vinh",
    "phone_code": 294
  },
  {
    "code": "86",
    "name": "Tỉnh Vĩnh Long",
    "division_type": "tỉnh",
    "codename": "tinh_vinh_long",
    "phone_code": 270
  },
  {
    "code": "87",
    "name": "Tỉnh Đồng Tháp",
    "division_type": "tỉnh",
    "codename": "tinh_dong_thap",
    "phone_code": 277
  },
  {
    "code": "88",
    "name": "Tỉnh Sóc Trăng",
    "division_type": "tỉnh",
    "codename": "tinh_soc_trang",
    "phone_code": 299
  },
  {
    "code": "89",
    "name": "Tỉnh An Giang",
    "division_type": "tỉnh",
    "codename": "tinh_an_giang",
    "phone_code": 296
  },
  {
    "code": "91",
    "name": "Tỉnh Kiên Giang",
    "division_type": "tỉnh",
    "codename": "tinh_kien_giang",
    "phone_code": 297
  },
  {
    "code": "92",
    "name": "Thành phố Cần Thơ",
    "division_type": "thành phố trung ương",
    "codename": "thanh_pho_can_tho",
    "phone_code": 292
  },
  {
    "code": "93",
    "name": "Tỉnh Hậu Giang",
    "division_type": "tỉnh",
    "codename": "tinh_hau_giang",
    "phone_code": 293
  },
  {
    "code": "94",
    "name": "Tỉnh Sóc Trăng",
    "division_type": "tỉnh",
    "codename": "tinh_soc_trang",
    "phone_code": 299
  },
  {
    "code": "95",
    "name": "Tỉnh Bạc Liêu",
    "division_type": "tỉnh",
    "codename": "tinh_bac_lieu",
    "phone_code": 291
  },
  {
    "code": "96",
    "name": "Tỉnh Cà Mau",
    "division_type": "tỉnh",
    "codename": "tinh_ca_mau",
    "phone_code": 290
  }
];

// Lưu trữ districts và wards tạm thời trong bộ nhớ để tối ưu hiệu suất
let districtsCache = {};
let wardsCache = {};

// Dữ liệu quận/huyện của tất cả các tỉnh
const allDistricts = {
  "01": [
    { code: "001", name: "Quận Ba Đình", division_type: "quận", codename: "quan_ba_dinh", province_code: "01" },
    { code: "002", name: "Quận Hoàn Kiếm", division_type: "quận", codename: "quan_hoan_kiem", province_code: "01" },
    { code: "003", name: "Quận Tây Hồ", division_type: "quận", codename: "quan_tay_ho", province_code: "01" },
    { code: "004", name: "Quận Long Biên", division_type: "quận", codename: "quan_long_bien", province_code: "01" },
    { code: "005", name: "Quận Cầu Giấy", division_type: "quận", codename: "quan_cau_giay", province_code: "01" },
    { code: "006", name: "Quận Đống Đa", division_type: "quận", codename: "quan_dong_da", province_code: "01" },
    { code: "007", name: "Quận Hai Bà Trưng", division_type: "quận", codename: "quan_hai_ba_trung", province_code: "01" },
    { code: "008", name: "Quận Hoàng Mai", division_type: "quận", codename: "quan_hoang_mai", province_code: "01" },
    { code: "009", name: "Quận Thanh Xuân", division_type: "quận", codename: "quan_thanh_xuan", province_code: "01" },
    { code: "016", name: "Huyện Sóc Sơn", division_type: "huyện", codename: "huyen_soc_son", province_code: "01" },
    { code: "017", name: "Huyện Đông Anh", division_type: "huyện", codename: "huyen_dong_anh", province_code: "01" },
    { code: "018", name: "Huyện Gia Lâm", division_type: "huyện", codename: "huyen_gia_lam", province_code: "01" },
    { code: "019", name: "Quận Nam Từ Liêm", division_type: "quận", codename: "quan_nam_tu_liem", province_code: "01" },
    { code: "020", name: "Huyện Thanh Trì", division_type: "huyện", codename: "huyen_thanh_tri", province_code: "01" },
    { code: "021", name: "Quận Bắc Từ Liêm", division_type: "quận", codename: "quan_bac_tu_liem", province_code: "01" },
    { code: "250", name: "Huyện Mê Linh", division_type: "huyện", codename: "huyen_me_linh", province_code: "01" },
    { code: "268", name: "Quận Hà Đông", division_type: "quận", codename: "quan_ha_dong", province_code: "01" },
    { code: "269", name: "Thị xã Sơn Tây", division_type: "thị xã", codename: "thi_xa_son_tay", province_code: "01" },
    { code: "271", name: "Huyện Ba Vì", division_type: "huyện", codename: "huyen_ba_vi", province_code: "01" },
    { code: "272", name: "Huyện Phúc Thọ", division_type: "huyện", codename: "huyen_phuc_tho", province_code: "01" },
    { code: "273", name: "Huyện Đan Phượng", division_type: "huyện", codename: "huyen_dan_phuong", province_code: "01" },
    { code: "274", name: "Huyện Hoài Đức", division_type: "huyện", codename: "huyen_hoai_duc", province_code: "01" },
    { code: "275", name: "Huyện Quốc Oai", division_type: "huyện", codename: "huyen_quoc_oai", province_code: "01" },
    { code: "276", name: "Huyện Thạch Thất", division_type: "huyện", codename: "huyen_thach_that", province_code: "01" },
    { code: "277", name: "Huyện Chương Mỹ", division_type: "huyện", codename: "huyen_chuong_my", province_code: "01" },
    { code: "278", name: "Huyện Thanh Oai", division_type: "huyện", codename: "huyen_thanh_oai", province_code: "01" },
    { code: "279", name: "Huyện Thường Tín", division_type: "huyện", codename: "huyen_thuong_tin", province_code: "01" },
    { code: "280", name: "Huyện Phú Xuyên", division_type: "huyện", codename: "huyen_phu_xuyen", province_code: "01" },
    { code: "281", name: "Huyện Ứng Hòa", division_type: "huyện", codename: "huyen_ung_hoa", province_code: "01" },
    { code: "282", name: "Huyện Mỹ Đức", division_type: "huyện", codename: "huyen_my_duc", province_code: "01" }
  ],
  "88": [
    // ✅ Placeholder: Sóc Trăng (code 88) - districts will be added later
    { code: "881", name: "Thành phố Sóc Trăng", division_type: "thành phố", codename: "thanh_pho_soc_trang", province_code: "88" },
    { code: "882", name: "Huyện Châu Thành", division_type: "huyện", codename: "huyen_chau_thanh", province_code: "88" },
    { code: "883", name: "Huyện Kế Sách", division_type: "huyện", codename: "huyen_ke_sach", province_code: "88" },
    { code: "884", name: "Huyện Mỹ Tú", division_type: "huyện", codename: "huyen_my_tu", province_code: "88" },
    { code: "885", name: "Huyện Cù Lao Dung", division_type: "huyện", codename: "huyen_cu_lao_dung", province_code: "88" },
    { code: "886", name: "Huyện Long Phú", division_type: "huyện", codename: "huyen_long_phu", province_code: "88" },
    { code: "887", name: "Huyện Mỹ Xuyên", division_type: "huyện", codename: "huyen_my_xuyen", province_code: "88" },
    { code: "888", name: "Thị xã Ngã Năm", division_type: "thị xã", codename: "thi_xa_nga_nam", province_code: "88" },
    { code: "889", name: "Huyện Thạnh Trị", division_type: "huyện", codename: "huyen_thanh_tri", province_code: "88" },
    { code: "890", name: "Thị xã Vĩnh Châu", division_type: "thị xã", codename: "thi_xa_vinh_chau", province_code: "88" },
    { code: "891", name: "Huyện Trần Đề", division_type: "huyện", codename: "huyen_tran_de", province_code: "88" }
  ],
  // Các tỉnh khác sẽ được thêm vào tương tự
};

/**
 * Hàm lấy danh sách quận/huyện theo mã tỉnh
 * @param {string} provinceCode - Mã tỉnh/thành phố
 * @returns {Array} - Mảng các quận/huyện thuộc tỉnh/thành phố
 */
export const getDistrictsOfProvince = (provinceCode) => {
  // Kiểm tra cache trước
  if (districtsCache[provinceCode]) {
    return districtsCache[provinceCode];
  }
  
  // Lấy dữ liệu từ allDistricts
  const districts = allDistricts[provinceCode] || [];
  
  // Lưu vào cache
  districtsCache[provinceCode] = districts;
  return districts;
};

// Dữ liệu xã/phường của tất cả các quận/huyện
const allWards = {
  "001": [ // Ba Đình, Hà Nội
    { code: "00001", name: "Phường Phúc Xá", division_type: "phường", codename: "phuong_phuc_xa", district_code: "001" },
    { code: "00004", name: "Phường Trúc Bạch", division_type: "phường", codename: "phuong_truc_bach", district_code: "001" },
    { code: "00006", name: "Phường Vĩnh Phúc", division_type: "phường", codename: "phuong_vinh_phuc", district_code: "001" },
    { code: "00007", name: "Phường Cống Vị", division_type: "phường", codename: "phuong_cong_vi", district_code: "001" },
    { code: "00008", name: "Phường Liễu Giai", division_type: "phường", codename: "phuong_lieu_giai", district_code: "001" },
    { code: "00010", name: "Phường Nguyễn Trung Trực", division_type: "phường", codename: "phuong_nguyen_trung_truc", district_code: "001" },
    { code: "00013", name: "Phường Quán Thánh", division_type: "phường", codename: "phuong_quan_thanh", district_code: "001" },
    { code: "00016", name: "Phường Ngọc Hà", division_type: "phường", codename: "phuong_ngoc_ha", district_code: "001" },
    { code: "00019", name: "Phường Điện Biên", division_type: "phường", codename: "phuong_dien_bien", district_code: "001" },
    { code: "00022", name: "Phường Đội Cấn", division_type: "phường", codename: "phuong_doi_can", district_code: "001" },
    { code: "00025", name: "Phường Ngọc Khánh", division_type: "phường", codename: "phuong_ngoc_khanh", district_code: "001" },
    { code: "00028", name: "Phường Kim Mã", division_type: "phường", codename: "phuong_kim_ma", district_code: "001" },
    { code: "00031", name: "Phường Giảng Võ", division_type: "phường", codename: "phuong_giang_vo", district_code: "001" },
    { code: "00034", name: "Phường Thành Công", division_type: "phường", codename: "phuong_thanh_cong", district_code: "001" }
  ],
  "881": [ // Thành phố Sóc Trăng
    { code: "88101", name: "Phường 1", division_type: "phường", codename: "phuong_1", district_code: "881" },
    { code: "88102", name: "Phường 2", division_type: "phường", codename: "phuong_2", district_code: "881" },
    { code: "88103", name: "Phường 3", division_type: "phường", codename: "phuong_3", district_code: "881" },
    { code: "88104", name: "Phường 4", division_type: "phường", codename: "phuong_4", district_code: "881" },
    { code: "88105", name: "Phường 5", division_type: "phường", codename: "phuong_5", district_code: "881" },
    { code: "88106", name: "Phường 6", division_type: "phường", codename: "phuong_6", district_code: "881" },
    { code: "88107", name: "Phường 7", division_type: "phường", codename: "phuong_7", district_code: "881" },
    { code: "88108", name: "Phường 8", division_type: "phường", codename: "phuong_8", district_code: "881" },
    { code: "88109", name: "Phường 9", division_type: "phường", codename: "phuong_9", district_code: "881" },
    { code: "88110", name: "Phường 10", division_type: "phường", codename: "phuong_10", district_code: "881" }
  ],
  "882": [ // Huyện Châu Thành
    { code: "88201", name: "Xã Hồ Đắc Kiện", division_type: "xã", codename: "xa_ho_dac_kien", district_code: "882" },
    { code: "88202", name: "Xã Phú Tâm", division_type: "xã", codename: "xa_phu_tam", district_code: "882" },
    { code: "88203", name: "Xã Thiện Mỹ", division_type: "xã", codename: "xa_thien_my", district_code: "882" }
  ],
  "883": [ // Huyện Kế Sách
    { code: "88301", name: "Xã Kế Sách", division_type: "xã", codename: "xa_ke_sach", district_code: "883" },
    { code: "88302", name: "Xã Xuân Hòa", division_type: "xã", codename: "xa_xuan_hoa", district_code: "883" }
  ],
  "884": [ // Huyện Mỹ Tú
    { code: "88401", name: "Xã Mỹ Tú", division_type: "xã", codename: "xa_my_tu", district_code: "884" },
    { code: "88402", name: "Xã Hưng Phú", division_type: "xã", codename: "xa_hung_phu", district_code: "884" }
  ],
  "885": [ // Huyện Cù Lao Dung
    { code: "88501", name: "Xã Cù Lao Dung", division_type: "xã", codename: "xa_cu_lao_dung", district_code: "885" },
    { code: "88502", name: "Xã An Thạnh 1", division_type: "xã", codename: "xa_an_thanh_1", district_code: "885" }
  ],
  "886": [ // Huyện Long Phú
    { code: "88601", name: "Xã Long Phú", division_type: "xã", codename: "xa_long_phu", district_code: "886" },
    { code: "88602", name: "Xã Song Phụng", division_type: "xã", codename: "xa_song_phung", district_code: "886" }
  ],
  "887": [ // Huyện Mỹ Xuyên
    { code: "88701", name: "Xã Mỹ Xuyên", division_type: "xã", codename: "xa_my_xuyen", district_code: "887" },
    { code: "88702", name: "Xã Đại Tâm", division_type: "xã", codename: "xa_dai_tam", district_code: "887" }
  ],
  "888": [ // Thị xã Ngã Năm
    { code: "88801", name: "Phường 1", division_type: "phường", codename: "phuong_1", district_code: "888" },
    { code: "88802", name: "Phường 2", division_type: "phường", codename: "phuong_2", district_code: "888" },
    { code: "88803", name: "Xã Vĩnh Quới", division_type: "xã", codename: "xa_vinh_quoi", district_code: "888" }
  ],
  "889": [ // Huyện Thạnh Trị
    { code: "88901", name: "Xã Thạnh Trị", division_type: "xã", codename: "xa_thanh_tri", district_code: "889" },
    { code: "88902", name: "Xã Phú Lộc", division_type: "xã", codename: "xa_phu_loc", district_code: "889" }
  ],
  "890": [ // Thị xã Vĩnh Châu
    { code: "89001", name: "Phường 1", division_type: "phường", codename: "phuong_1", district_code: "890" },
    { code: "89002", name: "Phường 2", division_type: "phường", codename: "phuong_2", district_code: "890" },
    { code: "89003", name: "Xã Lại Sơn", division_type: "xã", codename: "xa_lai_son", district_code: "890" }
  ],
  "891": [ // Huyện Trần Đề
    { code: "89101", name: "Xã Trần Đề", division_type: "xã", codename: "xa_tran_de", district_code: "891" },
    { code: "89102", name: "Xã Liêu Tú", division_type: "xã", codename: "xa_lieu_tu", district_code: "891" }
  ],
  // Các quận/huyện khác sẽ được thêm vào tương tự
};

/**
 * Hàm lấy danh sách xã/phường theo mã quận/huyện
 * @param {string} districtCode - Mã quận/huyện
 * @returns {Array} - Mảng các xã/phường thuộc quận/huyện
 */
export const getWardsOfDistrict = (districtCode) => {
  // Kiểm tra cache trước
  if (wardsCache[districtCode]) {
    return wardsCache[districtCode];
  }
  
  // Lấy dữ liệu từ allWards
  const wards = allWards[districtCode] || [];
  
  // Lưu vào cache
  wardsCache[districtCode] = wards;
  return wards;
};
